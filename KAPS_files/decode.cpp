...
int main()
{
  ...
  if (answer == "packet")
  {
    ...
    // Open the file to write the decoded to
    decodedFile.open("./decodedpackets/decodedpacket" + filename + ".txt");
    if (decodedFile.fail())
    {
      std::cout << "Error opening decoded packet file" << std::endl;
      return 1;
    }
  ...
  else
  {
    ...
    // Open the file to write the decoded to
    decodedFile.open("./decodeddata/decodeddata" + filename + ".txt");
    if (decodedFile.fail())
    {
      std::cout << "Error opening decoded packet file" << std::endl;
      return 1;
